...
const { Client, GatewayIntentBits, EmbedBuilder, ButtonBuilder, ButtonStyle, SlashCommandBuilder, ActionRowBuilder  } = Discord;
...
client.on('interactionCreate', async interaction => {
    if (interaction.isButton()) {
        // Vérifie si l'ID du bouton est 'Start'
        console.log("Bouton cliqué:", interaction.customId);
        if (interaction.customId === 'Start') {
            console.log("Tentative de réponse au bouton 'Start'");
        await interaction.reply({ content: 'test', ephemeral: true }).catch(console.error);
        }
    }
    ...
    const embed = new EmbedBuilder()
        .setColor('#0099ff') // Définit la couleur de l'embed à bleu
        .setTitle('TRIAL') // Optionnel: Ajoute un titre à l'embed
        .setDescription('24h activated'); // Le message que vous souhaitez envoyer
